...
const privateKey = fs.readFileSync(path.join(__dirname, '..', "keys", "paypay_rsa_private_key.pem"), 'utf8');

function executePayPayPayment(price, subject, order_id) {
    ...
    const encryptedBizContent = encryptAndBase64(JSON.stringify(bizContentData), privateKey);
    ...
    for (const key in requestBody) {
        if (requestBody.hasOwnProperty(key) && key !== 'sign' && key !== 'sign_type') {
...
function executeReferencePayment({ price, subject, order_id, timeout_express }) {
    function formatToDecimal(value) {
        return value.toFixed(2);
    }
...
function executeMulPayment({ price, subject, phone_num, quantity, order_id, timeout_express }) {
   
    console.log(price, subject, phone_num, quantity, order_id, timeout_express)
