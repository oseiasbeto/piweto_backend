...
module.exports = {
    async requestPayout(req, res) {
        try {
            ...
            if (moment().isBefore(moment(event.ends_at.date))) {
                return res.status(400).send({ message: "O evento ainda não terminou." });
            }
            ...
            const existingPayout = await Payout.findOne({ event_id, status: "pending" });
