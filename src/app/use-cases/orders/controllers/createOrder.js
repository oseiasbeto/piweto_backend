const User = require("../../../model/User") // Importa o modelo User para manipular dados de usuários
...
module.exports = { // Exporta o módulo com a função createOrder
    async createOrder(req, res) { // Define função assíncrona para criar um pedido
        try { // Inicia bloco try-catch para tratamento de erros
            const { // Desestrutura dados enviados no corpo da requisição
                full_name,
                email,
                phone,
                payment_method,
            ...
            const created_by = req.user.id // Obtém o ID do usuário autenticado que está criando o pedido
            const order_id = `${Date.now()}${Math.floor(Math.random() * 10000)}` // Gera um ID único para o pedido usando timestamp e número aleatório
            const code_ticket = `TICKET-${Date.now()}${Math.floor(Math.random() * 10000)}` // Gera um código único para o ingresso

            if (!event_id) return res.status(400).send({ // Verifica se o event_id foi fornecido
                message: "Informe o id do evento"
            })
            else { // Se event_id existe, prossegue
                const event = await Event.findOne({ // Busca o evento no banco pelo ID e status ativo ("a")
                    _id: event_id,
                    status: "a"
                })

                if (!event) return res.status(400).send({ // Se o evento não for encontrado, retorna erro
                    message: "Algo deu errado."
                })
                else if (event.tickets_purchased_count >= event.tickets_available_count) return res.status(400).send({ // Verifica se há ingressos disponíveis
                    message: "Ups! ja nao e possivel fazer uma reserva neste evento."
                })
                else if (!cart) return res.status(400).send({ // Verifica se o carrinho foi enviado
                    message: "Informe o carrinho de compras"
                })
                else { // Se todas as validações iniciais passaram
                    ...
                    if (amount > 0) { // Se o valor do pedido for maior que zero
                    ...
                    if (cart.batches.length) { // Se o carrinho contém lotes
                    ...
                    } else return res.status(400).send({ // Se o carrinho estiver vazio, retorna erro
                    ...
                    batches.forEach(async b => { // Para cada lote no carrinho
                        const batch = await Batch.findOne({ // Busca o lote no banco pelo ID
                            _id: b._id
                        })

                        if (!batch) return res.status(400).send({ // Se o lote não for encontrado, retorna erro
                            message: `Ups! nao achamos nenhum lote com este nome ${b.name}.`
                        })
                        else if (batch.quantity < b.quantitySelected) return res.status(400).send({ // Verifica se há ingressos suficientes no lote
                            message: `A quantidade selcionada e maior que a quantidade disponivel de ingressos no lote ${b.name}`
                        })
                    })

                    const user = await User.findOne({ // Busca o usuário criador do pedido
                        _id: created_by
                    })

                    if (!user) return res.status(400).send({ // Se o usuário não for encontrado, retorna erro
                        message: "Algo deu errado, faz o login e tente novamente!"
                    })

                    if (cart.amount == 0) { // Se o valor do carrinho for zero (ingressos grátis)
                        const newOrder = await Order.create({ // Cria um novo pedido no banco
                            id: order_id,
                            batches: batches,
                        ...
                        if (newOrder) { // Se o pedido foi criado com sucesso
                            batches.map(async (b) => { // Para cada lote no pedido
                                for (let i = 0; i < b.quantitySelected; i++) { // Cria um ingresso para cada unidade selecionada
                                    await Ticket.create({
                                        id: Date.now(),
                                        name: b.name,
                                        type: b.type,
                                    ...
                                    await Batch.updateOne({ // Atualiza a quantidade disponível no lote
                                        _id: b._id
                                    }, {
                                        $inc: {
                            ...
                            await event.updateOne({ // Atualiza o evento, reduzindo ingressos disponíveis
                                $inc: {
                            ...
                            res.status(200).send({ // Retorna sucesso para ingressos grátis
                        ...
                        switch (payment_method) { // Verifica o método de pagamento
                            case "reference": // Caso o método seja pagamento por referência
                                ...
                                await executeReferencePayment(data).then(async (response) => { // Executa o pagamento
                                    if (response.data.code == "S0001") { // Se o pagamento for bem-sucedido
                                        ...
                                        if (newOrder) { // Se o pedido foi criado
                                            ...
                                            if (user?.email) { // Se o usuário tiver e-mail, envia notificação
                                                sendMail(user.email, 'payment-ref', `Reserva iniciada para o evento ${event.name}`, {
                                                    ...
                                                    validity: moment(newOrder.expires_at).add("1", "h").format("YYYY/MM/DD HH:mm")
                                                })
                                            }

                                            // Envia mensagem (ex.: SMS) com detalhes do pagamento
                                            if (newOrder.data.phone) {
        ...
        } catch (err) { // Captura qualquer erro interno
            res.status(500).send({ // Retorna erro 500 com a mensagem do erro
                message: err.message
            })
        }
    }
